...
class Carry_vehicle_anomaly extends Admin_Controller {

    public function index()
    {
        $this->set_path('在途车辆异常列表');
            ...
            if ($data_list) {
                foreach ($data_list as $vehicle_data) {
                    if (in_array($vehicle_data['driver_id'], $driver_ids)) {
                        $carry_driver_ids[$vehicle_data['driver_id']] = $vehicle_data['driver_id'];
                    }
                }
            }
        }
        ...
        $this->data['links'] = '';
        if (!empty($carry_driver_ids)) {
            $where = array(
                'driver_id' => $carry_driver_ids,
            );
            ...
            $page_config['base_url'] = site_url(''.$this->appfolder.'/carry_vehicle_anomaly/index/'.$search_str);
            ...
            if ($this->data['data_list']) {
                foreach ($this->data['data_list'] as &$value) {
                    ...
                    // 运单公司
                    $shipper_data = $this->shipper_service->get_shipper_by_id($value['order_data']['shipper_id']);
        ...
        $this->load->view(''.$this->appfolder.'/vehicle/carry_vehicle_anomaly_view', $this->data);
    }
}
