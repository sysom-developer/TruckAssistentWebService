...
class Order extends Public_MY_Controller {
    ...
    public function ajax_get_order_detail()
    {
        ...
        $this->data['show_interval_track'] = $this->input->post('show_interval_track', TRUE);
        ...
        if ($data_list) {
            foreach ($data_list as $key => $value) {
                ...
                if ($value['order_type'] == 4) {
                    ...
                    // 间隔 4 小时显示当前位置
                    if ($this->data['show_interval_track'] == 1) {
                        $where = array(
                            'driver_id' => $value['driver_id'],
                            'UNIX_TIMESTAMP(create_time) >=' => strtotime($value['create_time']),
                        );
                        $track_data_list = $this->tracking_service->get_tracking_data_list($where);
                        if ($track_data_list) {
                            $interval_time = 4 * 3600; // 4小时
                            $i = 0;
                            foreach ($track_data_list as $track_value) {
                                $create_time = strtotime($track_value['create_time']);

                                if ($i == 0)
                                    $next_time = $interval_time * $i + $create_time;

                                if ($create_time >= $next_time) {
                                    if (empty($track_value['latitude']) || empty($track_value['longitude'])) {
                                        continue;
                                    }

                                    $location = get_location_by_lat_lng($track_value['latitude'], $track_value['longitude']);
                                    $track_desc = '在  '.$location;

                                    $track_data['track_log'][] = array(
                                        'track_desc' => $track_desc,
                                        'track_time' => tran_time(strtotime($track_value['create_time'])),
                                        'time' => strtotime($track_value['create_time']),
                                    );
                    ...
                    if ($driver_anomaly_data_list) {
                        foreach ($driver_anomaly_data_list as $value2) {
                            ...
                            $driver_anomaly[] = array(
                                ...
                                'province_name' => $value2['province_name'] != $value2['city_name'] ? $value2['province_name'] : '',
                                'city_name' => $value2['city_name'],
                                'speedInKPH' => intval($value2['speedInKPH']),
                                'heading' => $value2['heading'],
                                'cretime' => tran_time($value2['cretime']),
            ...
            $track_data['track_log'] = multi_array_sort($track_data['track_log'], 'time', SORT_DESC);
        }
    ...
    public function history_finish_order()
    {
        ...
        if (!empty($this->data['shipper_driver_count_data']['driver_ids'])) {
            ...
            $this->data['search_str'] .= '&history_type='.$this->data['history_type'].'&order_type='.$this->data['order_type'];
            ...
            if ($this->data['data_list']) {
                ...
                foreach ($this->data['data_list'] as &$value) {
                    $value['order_type'] = $order_type_desc[$value['order_type']];
                 ...
                 $this->data['data_list'] = multi_array_sort($this->data['data_list'], 'order_id', SORT_DESC);
            }
        }
    ...
    public function ajax_get_track_driver_route()
    {
        ...
        $error['order_data'] = $this->orders_service->get_orders_by_id($order_id);
        $error['order_data']['good_end_time'] = date('m-d H:i', $error['order_data']['good_end_time']);

        $time = time();

        // 当前位置信息
        $where = array(
            'driver_id' => $error['order_data']['driver_id'],
            'vehicle_id' => $error['order_data']['vehicle_id'],
        );
        $tracking_data = $this->tracking_service->get_tracking_data($where, 1, 0, 'id', 'DESC');
        ...
        // 可用是否异常
        if ($error['order_data']['order_type'] == 3) {
            // 24 小时位置未上报
        ...
        // 在途是否异常
        if ($error['order_data']['order_type'] == 4) {
            // 24 小时位置未上报
            if (($time - $tracking_unix_time) >= (24 * 3600)) {
                $is_anomaly = 1;
            // 运单预计到达时间已过当前时间 6 小时
            } elseif (($time - $error['order_data']['good_end_time']) >= (6 * 3600)) {
                $is_anomaly = 1;
            // 无 tracking 数据
            } elseif (empty($tracking_data)) {
                $is_anomaly = 1;
        ...
        $order_type_desc = $this->config->item('order_type_desc');
        $error['order_type_desc'] = $order_type_desc[$error['order_data']['order_type']];

        $error['driver_data'] = $this->driver_service->get_driver_by_id($error['order_data']['driver_id']);

        $where = array(
            'driver_id' => $error['order_data']['driver_id'],
            'UNIX_TIMESTAMP(create_time) >=' => $error['order_data']['good_start_time'],
        );
        $track_data_list = $this->tracking_service->get_tracking_data_list($where);
        if ($track_data_list) {
            ...
            foreach ($track_data_list as $value) {
                ...
                if ($create_time >= $next_time) {
                    ...
                    $error['current_lat'] = $value['latitude'];
                    $error['current_lng'] = $value['longitude'];
                    $current_location = get_location_by_lat_lng($value['latitude'], $value['longitude']);
                    $error['current_location'] = $current_location.' '.date('m-d H:i', strtotime($value['create_time']));
    ...
    public function ajax_get_history_track_driver_route()
    {
        $error = array(
            'code' => 'success',
