                ...
                for (var i = 0; i < json.track_data.track_log.length; i++) {
                    ...
                    if (driver_anomaly != undefined) {
                        for (var j = 0; j < driver_anomaly.length; j++) {
                            ...
                            var driver_anomaly_text = '在 '+driver_anomaly[j].province_name+''+driver_anomaly[j].city_name+' '+driver_anomaly[j].exce_desc+'，时速：'+driver_anomaly[j].speedInKPH+'';
...
<div class="order_detail order-detail-layer" style="display: none;">
    ...
    <div class="savebut order-detail-layer-btn" style="width:940px;">
        <input type="button" class="cancel order-tracking-btn" value="运单跟踪">
        <input type="button" class="cancel order-contract-btn" value="运输合同">
    </div>
</div>
<!--一般货主查看详情结束-->
