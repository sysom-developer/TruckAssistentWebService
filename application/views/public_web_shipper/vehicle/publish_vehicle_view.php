...
$(function() {
    ...
    // 搜索手机号码
    $("input[name=driver_mobile]").keyup(function() {
        $(".error-tip").hide();
        $(".autocomplete-driver-mobile-data").hide();

        var driver_mobile = $(this).val();

        $(".driver-mobile-loading").show();

        $.post(apppath + fetch_class + '/ajax_search_driver_mobile', {driver_mobile: driver_mobile}, function(json) {
            $(".error-tip").css("background", "#f26b54").hide();
            if (json.code == 'success') {
                var html = '';
                ...
                $(".autocomplete-driver-mobile-data").html(html);
                $(".autocomplete-driver-mobile-data").show();
                $(".driver-mobile-loading").hide();
            } else {
                $(".error-tip").find("span").html(json.code);
                $(".error-tip").show();
    ...
    //选中标签
    $(".autocomplete-driver-mobile-data").delegate("a", "click", function() {
        var vehicle_card_num = $(this).attr("vehicle_card_num");
        var driver_name = $(this).attr("driver_name");
        var driver_mobile = $(this).attr("driver_mobile");
        var vehicle_type_id = $(this).attr("vehicle_type_id");
        ...
        $(this).parent("div.autocomplete-driver-mobile-data").hide();
    });
    ...
    $(".publish-vehicle-submit").click(function() {
        ...
        $.post(apppath + 'vehicle/ajax_do_publish_vehicle', $("form[name=publish_vehicle_form]").serialize(), function(json) {
            if (json.code == 'success') {
                ...
                $(".error-tip").find("span").css("background", "#5fc53d");
                $(".error-tip").show();
                window.setTimeout("window.location.reload();", 1000);
            } else {
                _this.attr('disabled', false);
                $(".error-tip").find("span").html(json.code);
                $(".error-tip").find("span").css("background", "#f26b54");
                $(".error-tip").show();
...
<div class="editcar" style="display:none">
    ...
    <div class="carinfo">
        ...
        <dl>
            <dd>联系电话</dd>
            <dt>
                <input name="driver_mobile" type="text" val="请输入联系电话" value="请输入联系电话" class="input placeholder" autocomplete="off">
                <span class="driver-mobile-loading" style="display: none;"><img src="<?php echo static_url('static/images/loading.gif')?>"></span>
                <div class="item_list autocomplete-driver-mobile-data" style="width: 310px;">
                </div>
            </dt>
        </dl>
        <dl>
