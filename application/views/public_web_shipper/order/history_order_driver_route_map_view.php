...
var truck_driver_icon = '<?php echo static_url("static/images/'+appfolder+'/history_map_red_vehicle.png")?>';
...
$(function() {
    ...
    $('.history-order-show-driver-route').click(function() {
        ...
        $.post(apppath + 'order/ajax_get_history_track_driver_route', {order_id: order_id}, function(json) {
...
function init_history_order_driver_route_detail() {
    ...
    var marker = new BMap.Marker(first_pts_var,{icon:startMarkerIcon});  // 创建标注
    map.addOverlay(marker);

    var marker = new BMap.Marker(last_pts_var,{icon:endMarkerIcon});  // 创建标注
    map.addOverlay(marker);

    var driver_route_icon = new BMap.Icon(truck_driver_icon, new BMap.Size(35, 58), {    //小车图片
